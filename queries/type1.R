...
## The two inputs.
facts <- Load(fb_facts)
rules <- ReadCSV("/pcluster/freebase/rules/rules1.csv", sep = " ", line.number = Rule,
...
Store(join, groupjoin_intermediates, Object = objectID, Subject = subjectID, Predicate = head, .overwrite = TRUE)
