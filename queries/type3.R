...
counts1 <- Segmenter(GroupBy(Load(fb_facts), c(arg1 = subjectID, predicate), count1 = Count()))
counts2 <- Segmenter(GroupBy(Load(fb_facts), c(arg2 = subjectID, predicate), count2 = Count()))

rules <- ReadCSV("/pcluster/freebase/rules/rules3.csv", sep = " ", line.number = ID,
...
groupjoin <- GroupJoin(list(facts, rules), c(Subject, Predicate, Object, Rule), rule = 3, frag = 1E4)
