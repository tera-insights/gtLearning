...
ops <- Segmenter(Group(Load(fb_facts), c(objectID, predicate = predicate$GetID()), subjectID))
spo <- Segmenter(Group(Load(fb_facts), c(subjectID, predicate = predicate$GetID()), objectID))
...
agg <- GroupBy(data, c(ID, head, body1, body2), fragment.size = 200, Gather(c(object1, count1, count2)),
...
rules <- Segmenter(Group(data, c(body1 = body1$GetID(), body2 = body2$GetID(), head = head$GetID(), ID),
                         object1, use.array = TRUE))

## Step 3 / Algorithm 4. The Group-Join GIST.
groupjoin <- GroupJoin(list(rules, ops, spo), c(Subject, Predicate, Object, Rule), frag = 1E4)
